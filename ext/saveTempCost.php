...
for($i=0;$i<sizeof($lineItems);$i++){
  ...
  if($_POST["SaleItemAdjustment_".$i] != "" && $_POST["SaleItemAdjustment_".$i] != 0){
    ...
    array_push($lineItemADJs, [$lineItemIDFull[0], $_POST["SaleItemAdjustment_".$i], $adjList->lineItems[$i][1]]);
  }

  if($_POST["taxItemExempt_".$i] != "" && $_POST["taxItemExempt_".$i] != 0){

    array_push($taxExempt, [$lineItemIDFull[0], $_POST["taxItemExempt_".$i]]);
  }
}
...
for($l=0;$l<sizeof($lineItemADJs);$l++){
  $queryLineItemAdj = "INSERT INTO lineItemADJ (lidID, stID, stSftID, businessDate, adjAmount, adjNote) VALUES
                (
                  ".$lineItemADJs[$l][0].",
                  ".$station.",
                  ".$shift.",
...
for($l=0;$l<sizeof($taxExempt);$l++){
  $queryTaxExempt = "INSERT INTO taxExemptSales (lidID, stID, stSftID, businessDate, tesAmount) VALUES
                (
                  ".$taxExempt[$l][0].",
                  ".$station.",
                  ".$shift.",
